    ...
    if (urlHandle && !autoSubmitted && !analyze.isPending && !analyze.data) {
      ...
      analyze.mutate({
        data: {
          ...
          style: prefs.aiStyle,
          depth: prefs.aiDepth,
          creativity: prefs.aiCreativity,
          focusAreas: prefs.aiFocusAreas,
        },
      });
    }
  }, []);
        ...
        <motion.div variants={fadeUp}>
          <div className="flex items-start gap-2 rounded-lg border border-amber-500/20 bg-amber-500/8 px-4 py-2.5 text-xs text-amber-300/80">
            ...
            <span>
              ...
              AI personality is set to <span className="font-semibold capitalize">{loadPrefs().aiPersonality}</span>. 
