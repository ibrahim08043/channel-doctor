...
function ConnectInner() {
  ...
  // ── Start YouTube OAuth ──────────────────────────────────────────────────
  // Opens a POPUP so the OAuth flow works even when this page is inside a
  // sandboxed Replit preview iframe. window.location.href is silently blocked
  // in sandboxed frames; window.open() (user-gesture popup) is allowed.
  // The popup completes the full Google consent flow, then posts a message back.
  const handleConnectYouTube = () => {
    const returnUrl = `${window.location.origin}${base}/connect`;
    setPhase({ id: "starting_oauth" });
    const oauthUrl = `/api/youtube-oauth/start?${new URLSearchParams({ returnUrl })}`;
    const popup = window.open(oauthUrl, "youtube-oauth", "width=600,height=700,popup=yes,noopener=no");
    if (!popup) {
      // Pop-up was blocked — fall back to same-window navigation
  ...
  if (autoDetectSettled && ((me.data?.channelId && phase.id !== "success") ||
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        {thumb && <img src={thumb} alt="" className="mx-auto h-20 w-20 rounded-full ring-2 ring-primary/30" />}
        ...
        <Card className="p-5 glass">
          <div className="flex items-center gap-4">
            {phase.channelThumbnail ? (
              <img src={phase.channelThumbnail} alt="" className="h-16 w-16 rounded-full ring-2 ring-primary/30 shrink-0" />
            ) : (
        ...
        <div>
          <Card className="p-6 glass space-y-4">
            ...
            <Button
              ...
              disabled={phase.id === "starting_oauth"}
            >
